...
func (c *Command) Get() (int64, error) {
	...
	if err != nil {
		return 0, errors.New(fmt.Sprintf("Error finding all documents: %s", err.Error()))
	}
	...
	if err != nil {
		return 0, errors.New(fmt.Sprintf("Decode error: %s", err.Error()))
	}
...
func (c *Command) GetOne() error {
	...
	if err != nil {
		return errors.New(fmt.Sprintf("Error finding document: %s", err.Error()))
	}
